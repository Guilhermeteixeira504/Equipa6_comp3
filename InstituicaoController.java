...
public class InstituicaoController {
	...
	public List<InstituicaoDTO> getAll() {
		return instituicaoService.getAllInstituicoes()
				.stream().map(InstituicaoMapper::toDTO).toList();
	}
	
	@GetMapping("/{id}")
	public InstituicaoDTO getById(@PathVariable Long id) {
		return InstituicaoMapper.toDTO(instituicaoService.getInstituicao(id));
	}
	
	@PostMapping
	public InstituicaoDTO create(@RequestBody InstituicaoDTO inst) {
		Instituicao instituicao = instituicaoService.createInstituicao(InstituicaoMapper.toEntity(inst));
		return InstituicaoMapper.toDTO(instituicao);
	}
	
	@PutMapping("/{id}")
	public InstituicaoDTO update(@PathVariable Long id, @RequestBody InstituicaoDTO dto) {
		Instituicao updated = instituicaoService.updateInstituicao(id, InstituicaoMapper.toEntity(dto));
